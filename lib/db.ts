...
let jsonDbCache: { mtimeMs: number; db: FarmDatabase } | null = null;

function invalidateJsonDbCache(): void {
  jsonDbCache = null;
}
...
function loadJsonDb(): FarmDatabase {
  ...
  if (jsonDbCache && jsonDbCache.mtimeMs === mtimeMs) {
    return jsonDbCache.db;
  }
